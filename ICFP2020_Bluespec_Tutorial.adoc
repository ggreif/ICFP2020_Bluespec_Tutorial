...
BH:: Bluespec Haskell, also known as Bluespec Classic
...
fshow, it just prints the value of the bit representation, but with
...
IMPORTANT: Our choice of yellow round-cornered rectangles for
  ...
  as the conditions on the methods it invokes.  The overall body of a
...
* This is _quite different_ from the situation with so-called HLS
  (High Level Synthesis).  In HLS, the source code is C/{cpp} (with the
  execution semantics of C/{cpp}), and the HLS tool produces a
...
The sorting behavior is expressed by a series of 4 rules:
...
(synthesis of hardware from and algorithm specified in C/{cpp}).
